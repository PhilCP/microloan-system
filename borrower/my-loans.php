...
<main class="dashboard-main" id="dashboardMain" style="margin-left:240px;transition:0.3s;padding:30px;">
    ...
    <div class="filters">
        <?php foreach (['all','pending','approved','rejected','completed'] as $s): ?>
            <a href="?filter=<?php echo $s; ?>"
               class="filter-btn <?php echo $filter === $s ? 'active' : ''; ?>">
    ...
    <div class="loans-container">
        <?php if ($result->num_rows > 0): ?>
            ...
            <div class="loan-card" data-loan-id="<?php echo $loan['id']; ?>">
                ...
                <?php if (!empty($loan['id_number']) || !empty($loan['id_document'])): ?>
                <?php
                    $idV = (int)($loan['id_verified'] ?? 0);
                    if ($idV === 1) {
                        $idBorderColor = '#22c55e';
                        $idBg          = 'rgba(34,197,94,0.05)';
                        $idBadge       = '<span style="background:rgba(34,197,94,.12);color:#22c55e;border:1px solid rgba(34,197,94,.3);font-size:10px;font-weight:800;text-transform:uppercase;padding:2px 8px;border-radius:4px;">✓ Verified by Admin</span>';
                        $idMsg         = '<span style="color:#22c55e;font-size:12px;">Your ID has been verified. Application is being processed.</span>';
                    } elseif ($idV === -1) {
                        $idBorderColor = '#ef4444';
                        $idBg          = 'rgba(239,68,68,0.05)';
                        $idBadge       = '<span style="background:rgba(239,68,68,.1);color:#ef4444;border:1px solid rgba(239,68,68,.3);font-size:10px;font-weight:800;text-transform:uppercase;padding:2px 8px;border-radius:4px;">✗ ID Rejected</span>';
                        $idMsg         = '<span style="color:#f87171;font-size:12px;">Your ID number does not match the uploaded document. Please contact support or re-apply with the correct ID.</span>';
                    } else {
                        $idBorderColor = '#3b82f6';
                        $idBg          = 'rgba(59,130,246,0.04)';
                        $idBadge       = '<span style="background:rgba(234,179,8,.1);color:#eab308;border:1px solid rgba(234,179,8,.25);font-size:10px;font-weight:800;text-transform:uppercase;padding:2px 8px;border-radius:4px;">⏳ Pending Admin Review</span>';
                        $idMsg         = '<span style="color:#aaa;font-size:12px;">Your document has been received and is awaiting admin verification.</span>';
                    }
                ?>
                <div style="background:<?php echo $idBg; ?>;border:1px solid <?php echo $idBorderColor; ?>;border-left:3px solid <?php echo $idBorderColor; ?>;border-radius:8px;padding:12px 15px;margin-bottom:15px;">
                    <div class="detail-label" style="color:<?php echo $idBorderColor; ?>;margin-bottom:8px;">🪪 Identity Verification</div>
                    <div style="font-size:13px;color:#aaa;margin-bottom:6px;">
                        <?php if (!empty($loan['id_number'])): ?>
                            ID No: <strong style="color:#fff;"><?php echo htmlspecialchars($loan['id_number']); ?></strong>
                            &nbsp;•&nbsp;
                        <?php endif; ?>
                        <?php echo $idBadge; ?>
                    </div>
                    <div><?php echo $idMsg; ?></div>
                </div>
                <?php endif; ?>
